...
- **No public API change.** `html\`...\`` is identical. `ParsedHTML` gains `result: string`
  and `fragment` becomes `DocumentFragment | null`.
...
1. **Struct extraction (mechanical).** Move the module `let`/`const` state and nine buffers
   ...
   the per-char cursor cost). The `parsing - cached` group is the control: it should *not*
...
2. **Tail + materialization split.** `parse()` returns `result` + `fragment: null`. Add
   ...
   **Bench note:** this step *relocates* the `innerHTML`→fragment work from `parse()` into
   first `setup()`. Expect `html.bench.ts > parsing (cold)` to get *faster* and
...
- Rewiring SSR to drop happy-dom from the render path — Part A only decouples *parse*.
