...
**Bench-gate discipline** (from `VALUE_KIND`): every change pays into *perf* or *clarity* and
...
- [ ] `[S]` **`await update()` hangs on root supersession of an in-flight child.** Verified live
  (`generator-layer.ts`). Outer supersedes a parked child via `stopInnerGenerator` → `cancelRun`
  never calls `finishUpdate`, and the outer's `signalRunFinished` skips resolution (`parent ===
  null`) → DOM lands correctly but the `update()` promise never resolves. Fix: funnel every terminal
  (done/throw/cancel) through `finishUpdate`; on supersession resolve once the superseding DOM has
  painted.
...
- [ ] `[M]` **Fold Scheduler into Producer** (`scheduler.ts`): move `flushPromise`/`dirty` onto the
  Producer, delete the module + per-connect rebuild. Nuance: the server's null-`#scheduler` no-op
  gate needs an explicit flag. Judgment call.
- [ ] `[S]` **Dead code:** `UPDATE_STATE`, `RUNTIME_KIND` (`constants.ts`); `TagBinding.endValues`
  (written at parse time, never read).
- [ ] `[S]` *(cosmetic)* `RawContentBinding` + `ContentBinding` are structurally identical — one
...
- [ ] `[M]` **Extract one `showFatal(host, error)`** — collapse the duplicated
  `abortAndShowError`/`reportSSRError` bodies. *(Maybe already resolved by the rework — confirm
  there's no second SSR-side display path, then drop if none.)*
...
- [ ] `[L]` *(lower)* Allow styles added directly as a class on a component / an alternate style
  registration.
...
- **`#flush` is O(all bindings).** A dirty-index queue makes it O(dirty) but trades a cache-friendly
  `Uint8Array` scan for a per-update allocation — only wins when binding count is high *and* dirty
