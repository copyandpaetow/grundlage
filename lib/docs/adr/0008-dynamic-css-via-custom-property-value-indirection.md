...
and routes each dynamic *value* through a CSS custom property updated per-instance with
...
The reparse *is* the bug. Measured in the cube editor: a per-frame hole that re-serializes
...
invalidation is the platform's *designed* primitive for dynamic values — browsers optimize
...
`update()` stays the only writer — this changes *how* a dirty binding applies, not *who*
triggers it.
...
CSS" here means "fast dynamic *values*," not "all dynamic CSS."

## Considered Options

- **Re-serialize `textContent` (status quo, rejected as the mechanism — kept as the
  fallback).** It *is* the measured bug. Retained only as the always-correct path for
  structural holes.
- **Rule-index CSSOM mutation (deferred, demoted).** The "thorough" approach: mutate
  ...
  indices drift as the sheet changes), and is a strict *superset* of this work — it needs the
  same analyzer/plan/dirty-diff and only *adds* span machinery. If ever built, it would
  handle only the *structural* holes that currently fall back, riding the same analyzer —
...
- **`adoptedStyleSheets` / constructable stylesheets (set aside — not a true alternative to
  ...
  the *static* sheet; per-instance values differ between instances and would *still* require
...
- **`css\`\`` composition is a separate, later concern.** v1 is shaped so a future `css\`\``
  (splicing one stylesheet into another) is additive, but that is about *composition* of
