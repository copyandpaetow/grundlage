...
describe.skipIf("happyDOM" in globalThis)("server-side rendering", () => {
     ...
     */
    const serverRender = async (tag: string, ComponentClass: ReturnType<typeof render>): Promise<string> => {
    ...
    describe("serialization", () => {
        ...
        test("serializes dynamic content with current values", async () => {
            ...
            customElements.define(tag, MyElement);
            const element = document.createElement(tag) as InstanceType<typeof MyElement>;
            document.body.appendChild(element);
            await sleep();
    ...
    describe("hydration", () => {
        ...
        test("preserves server-rendered DOM on initial mount", async () => {
            ...
            const makeComponent = () => render(function* () {
                yield () => html`<p>${text}</p>`;
            });

            const serialized = await serverRender(serverTag, makeComponent());
            const clientHTML = serialized.replace(new RegExp(serverTag, "g"), clientTag);
        ...
        test("cleanup callback fires after hydration", async () => {
            ...
            const serialized = await serverRender(serverTag, render(function* () {
                yield () => html`<p>content</p>`;
            }));
            const clientHTML = serialized.replace(new RegExp(serverTag, "g"), clientTag);
            const element = hydrateFromHTML(clientHTML);

            customElements.define(clientTag, render(function* () {
                yield () => html`<p>content</p>`;
                return () => {
                    cleaned = true;
                };
        ...
        test("attribute mutation triggers re-render after hydration", async () => {
            ...
            const makeComponent = () => render(function* (element) {
                yield () => html`<span>${element.getAttribute("data-label") ?? "none"}</span>`;
            });
