...
const sheetOf = (plan: ReturnType<typeof compileStyleSheet>) =>
	plan === null ? null : composeSheet(plan, plan.customPropertyNames);
...
describe("custom properties", () => {
	test("single value hole becomes one group and one var()", () => {
		const plan = compileStyleSheet([".box { color: ", 0, "; }"], TEMPLATE_HASH);
		expect(sheetOf(plan)).toBe(".box { color:var(--1a-0); }");
		expect(plan?.customPropertyNames).toEqual(["--1a-0"]);
		expect(plan?.customProperties).toEqual([
	...
	test("consecutive holes share the group of the first expression", () => {
		...
		expect(plan?.customProperties).toEqual([
			{ nameSuffix: 0, valueParts: [0, 1, "px"] },
		]);
	});
});
...
describe("naming", () => {
	test("a negative template hash yields no stray dash inside the name", () => {
		const plan = compileStyleSheet(["a{b:", 0, "}"], -1);
		expect(plan?.customPropertyNames[0]).toBe(
			`--${(4294967295).toString(36)}-0`,
		);
	});

	test("the name suffix is the expression index, not a per-binding counter", () => {
		const plan = compileStyleSheet([".a{color:", 2, "}"], TEMPLATE_HASH);
		expect(plan?.customPropertyNames[0]).toBe("--1a-2");
	});

	test("two bindings of one template get disjoint names with no shared state", () => {
		const firstPlan = compileStyleSheet([".a{color:", 0, "}"], TEMPLATE_HASH);
		const secondPlan = compileStyleSheet([".b{width:", 1, "}"], TEMPLATE_HASH);
		expect(firstPlan?.customPropertyNames[0]).toBe("--1a-0");
		expect(secondPlan?.customPropertyNames[0]).toBe("--1a-1");
	});
...
describe("!important", () => {
	...
	test("a top-level bang that is not !important bails", () => {
		expect(
			compileStyleSheet(["div{color:", 0, " !imp}"], TEMPLATE_HASH),
		).toBeNull();
	});

	test("two top-level bangs bail", () => {
		const parts = ["div{color:", 0, " !important !important}"];
		expect(compileStyleSheet(parts, TEMPLATE_HASH)).toBeNull();
	});

	test("a bang before a hole bails", () => {
		expect(
			compileStyleSheet(["div{color: x !", 0, "}"], TEMPLATE_HASH),
		).toBeNull();
	});
...
describe("structural bails", () => {
	test("a hole in a selector bails", () => {
		expect(
			compileStyleSheet(["div.", 0, "{color:red}"], TEMPLATE_HASH),
		).toBeNull();
	});

	test("a hole in a property name bails", () => {
		expect(compileStyleSheet(["div{", 0, ":red}"], TEMPLATE_HASH)).toBeNull();
	});
	...
	test("a whole-stylesheet hole bails", () => {
		expect(compileStyleSheet(["", 0, ""], TEMPLATE_HASH)).toBeNull();
	});
	...
	test("an unclosed rule bails", () => {
		expect(compileStyleSheet(["div{color:", 0, ";"], TEMPLATE_HASH)).toBeNull();
	});
...
describe("descriptor at-rules", () => {
	test("a hole inside @font-face bails — var() does not substitute there", () => {
		expect(
			compileStyleSheet(["@font-face{src:", 0, "}"], TEMPLATE_HASH),
		).toBeNull();
	});
	...
	test("a hole after a closed descriptor block is fast again", () => {
		...
		const plan = compileStyleSheet(parts, TEMPLATE_HASH);
	...
	test("a hole in a keyframe declaration is fast", () => {
		...
		expect(plan?.customProperties).toEqual([
			{ nameSuffix: 0, valueParts: ["rotate(", 0, "deg)"] },
		]);
	});
...
describe("strings and comments", () => {
	test("a hole inside a string bails", () => {
		expect(
			compileStyleSheet(['div{content:"', 0, '"}'], TEMPLATE_HASH),
		).toBeNull();
	});
	...
	test("a hole inside a comment bails", () => {
		expect(compileStyleSheet(["div{/* ", 0, " */}"], TEMPLATE_HASH)).toBeNull();
	});
	...
	test("an unterminated string bails", () => {
		expect(
			compileStyleSheet(['div{content:"a}', 0, ""], TEMPLATE_HASH),
		).toBeNull();
	});
});
...
describe("parentheses", () => {
	...
	test("an unclosed parenthesis bails", () => {
		expect(
			compileStyleSheet([".a{width:calc(", 0, "}"], TEMPLATE_HASH),
		).toBeNull();
	});
});
