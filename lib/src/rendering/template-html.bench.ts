...
describe("html() — tagged template creation", () => {
    ...
    bench("animation-shape template (20 style attrs, ~80 expressions)", () => {
        html`
            <div style="width:${1}%;background:hsl(${2},70%,${3}%);opacity:${4}"></div>
            <div style="width:${5}%;background:hsl(${6},70%,${7}%);opacity:${8}"></div>
            <div style="width:${9}%;background:hsl(${10},70%,${11}%);opacity:${12}"></div>
            <div style="width:${13}%;background:hsl(${14},70%,${15}%);opacity:${16}"></div>
            <div style="width:${17}%;background:hsl(${18},70%,${19}%);opacity:${20}"></div>
        `;
    });
});
...
describe("HTMLTemplate.update() — list reconciliation", () => {
    const items = Array.from({length: 20}, (_, index) => ({id: index, label: `item-${index}`}));
