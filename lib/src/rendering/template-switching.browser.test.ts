...
describe("template switching", () => {
    ...
    test("switches between three different template structures", async () => {
        ...
        const MyElement = render(function* () {
            yield () => {
                if (view === "list")
                    return html`<ul><li>item</li></ul>`;
                if (view === "detail")
                    return html`<article><h2>Title</h2><p>Body</p></article>`;
        ...
        await element.update();
        await sleep();
        expect(element.shadowRoot?.querySelector("li")?.textContent).toBe(
            "item",
        );
    ...
    test("switching templates preserves sibling content", async () => {
        ...
        const MyElement = render(function* () {
            yield () =>
                dynamic
                    ? html`<header>head</header><main>content-a</main><footer>foot</footer>`
                    : html`<header>head</header><main>content-b</main><footer>foot</footer>`;
        });
