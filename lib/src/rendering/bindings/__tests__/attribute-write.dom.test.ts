...
describe("applyAttributeValue - event listeners", () => {
	test("attaches an event listener for an on* key whose value is a function", () => {
		...
		applyAttributeValue(element, "onclick", handler);
	...
	test("removes the previous listener and attaches the new one when both are functions", () => {
		...
		applyAttributeValue(element, "onclick", firstHandler);
		applyAttributeValue(element, "onclick", secondHandler, firstHandler);
	...
	test("detaches the listener when value is null and oldValue is a function", () => {
		...
		applyAttributeValue(element, "onclick", handler);
		applyAttributeValue(element, "onclick", null, handler);
	...
	test("on-prefixed key that is not a real event property falls through to setAttribute", () => {
		...
		applyAttributeValue(element, "ondata", "payload");
...
describe("applyAttributeValue - exotic event names and CustomEvent payloads", () => {
	...
	test.each([
		...
		(attributeKey, eventName) => {
			...
			applyAttributeValue(element, attributeKey, handler);
	...
	test("CustomEvent detail reaches a listener bound to a standard event slot", () => {
		...
		applyAttributeValue(element, "oninput", (event: Event) => {
			received.push({
	...
	test("bubbling CustomEvent dispatched on a child fires a listener on the parent", () => {
		...
		applyAttributeValue(parent, "onclick", () => received.push("parent"));
	...
	test("detaches an exotic listener when the new value is null", () => {
		...
		applyAttributeValue(element, "ontransitionend", handler);
		applyAttributeValue(element, "ontransitionend", null, handler);
	...
	test("listener bound via mixed-case key still fires on the canonical event", () => {
		...
		applyAttributeValue(element, "onPointerDown", (event: Event) =>
	...
	test("custom event name with no matching prototype property lands as a JS property, not via addEventListener", () => {
		...
		applyAttributeValue(element, "onmycustomevent", handler);
	...
	test("removes the listener via null even when only oldValue is a function", () => {
		...
		//the entry guard in applyAttributeValue still has to enter the on* branch
		...
		applyAttributeValue(element, "onwheel", null, handler);
	...
	test("function → non-function on an event key detaches the listener and writes nothing", () => {
		...
		applyAttributeValue(element, "onclick", handler);
		applyAttributeValue(element, "onclick", "alert(1)", handler);
	...
	test("non-function → function on an event key binds once, with no leftover attribute", () => {
		...
		applyAttributeValue(element, "onclick", "alert(1)");
		expect(element.hasAttribute("onclick")).toBe(false);

		applyAttributeValue(element, "onclick", handler, "alert(1)");
		expect(element.hasAttribute("onclick")).toBe(false);
...
describe("applyAttributeValue - on- explicit listeners", () => {
	test("binds a custom event with no IDL property, skipping the in-element gate", () => {
		...
		applyAttributeValue(element, "on-form-reset", (event: Event) =>
			received.push(event.type),
		);
	...
	test("strips only the `on-` prefix, keeping hyphens in the event name", () => {
		...
		applyAttributeValue(element, "on-form-state-restore", (event: Event) =>
			received.push(event.type),
		);
	...
	test("swaps the handler, detaching the old listener", () => {
		...
		applyAttributeValue(element, "on-my-event", first);
		applyAttributeValue(element, "on-my-event", second, first);
	...
	test("detaches when the value becomes null", () => {
		...
		applyAttributeValue(element, "on-my-event", handler);
		applyAttributeValue(element, "on-my-event", null, handler);
	...
	test("lowercases the event name, matching the DOM's attribute-name folding", () => {
		...
		applyAttributeValue(element, "on-MyEvent", (event: Event) =>
			received.push(event.type),
		);
...
describe("applyAttributeValue - dead native handler warning", () => {
	test("warns when an on<name> function value finds no matching IDL property", () => {
		...
		applyAttributeValue(element, "onClik", () => {});
	...
	test("does not warn for a correctly spelled native handler", () => {
		...
		applyAttributeValue(element, "onclick", () => {});
	...
	test("does not warn for an on-<name> explicit custom-event listener", () => {
		...
		applyAttributeValue(element, "on-whatever", () => {});
	...
	test("does not warn for a non-on* name carrying a function", () => {
		...
		applyAttributeValue(element, "data-thing", () => {});
	...
	test("does not warn again on teardown when only oldValue is a function", () => {
		...
		applyAttributeValue(element, "onmadeup", null, () => {});
...
describe("applyAttributeValue - removal", () => {
	test.each([
		["null", null],
		["undefined", undefined],
		["false", false],
		...
		applyAttributeValue(element, "title", value);
		expect(element.hasAttribute("title")).toBe(false);
	});
});

describe("applyAttributeValue - stringable values", () => {
	test("sets a string attribute", () => {
		const element = document.createElement("div");
		applyAttributeValue(element, "title", "hello");
		expect(element.getAttribute("title")).toBe("hello");
	});

	test("coerces a number to a string attribute", () => {
		const element = document.createElement("input");
		applyAttributeValue(element, "tabindex", 5);
...
describe("applyAttributeValue - non-stringable values", () => {
	...
	test("assigns an array as a JS property", () => {
		...
		applyAttributeValue(element, "items", items);
	...
	test("removing a property-mode entry deletes the JS property (not just the attribute)", () => {
		...
		applyAttributeValue(element, "items", items);
		applyAttributeValue(element, "items", null, items);
	...
	test("calls update() when the receiver exposes one (custom-element handoff)", () => {
		...
		applyAttributeValue(element, "config", { nested: 1 });
...
describe("applyAttributeValue - stringable to non-stringable transition", () => {
	test("the JS property reflects the new non-stringable value", () => {
		const element = document.createElement("div");
		const payload = { nested: 1 };

		applyAttributeValue(element, "config", "previous-string");
		applyAttributeValue(element, "config", payload, "previous-string");
	...
	test("the prior stringable attribute is removed by the transition", () => {
		...
		applyAttributeValue(element, "config", { nested: 1 }, "previous-string");
	...
	test("a full stringable → object → stringable cycle leaves exactly one channel populated", () => {
		...
		applyAttributeValue(element, "config", { nested: 1 }, "first");
		expect(element.getAttribute("config")).toBe(null);
		expect(Object.hasOwn(element, "config")).toBe(true);

		applyAttributeValue(element, "config", "second", { nested: 1 });
	...
	test("the other channel is cleared without an oldValue — the two-argument setProp form", () => {
		...
		applyAttributeValue(element, "config", "second");
		expect(Object.hasOwn(element, "config")).toBe(false);
		expect(element.getAttribute("config")).toBe("second");
	});
	...
	test("update() still fires on the property-assignment branch after the transition", () => {
		...
		applyAttributeValue(element, "config", "before");
		expect(updateSpy).not.toHaveBeenCalled();
