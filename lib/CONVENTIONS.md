...
- **Errors propagate through one channel** (the `onError` handle). `try/catch` only at the
  boundaries that feed it; a deliberate swallow is `catch { /* why */ }`. One shared fatal
  display.
