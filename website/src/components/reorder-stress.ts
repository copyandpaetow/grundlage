...
customElements.define(
    ...
    render(function* (element) {
        ...
        const runOperation = (label: string, operation: (rows: Array<Row>) => void) => {
            operation(rows);
            ...
            const cumulativeMs = (previous?.averageDurationMs ?? 0) * (sampleCount - 1) + durationMs;
        ...
        yield () => html`
            ...
            <menu>
                <button onclick="${() => runOperation("reverse", reverseInPlace)}">reverse</button>
                <button onclick="${() => runOperation("shuffle", shuffleInPlace)}">shuffle</button>
                <button onclick="${() => runOperation("rotate", rotateOne)}">rotate 1</button>
                <button onclick="${() => runOperation("swap ends", swapEnds)}">swap ends</button>
                <button onclick="${() => runOperation("mutate", mutateValues)}">mutate values</button>
            </menu>
