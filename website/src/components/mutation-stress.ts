    ...
    render(function* (element) {
        ...
        yield () => html`
            ...
            <menu>
                ...
                <button onclick="${() => runOperation("unshift", unshiftOne)}">unshift</button>
                <button onclick="${() => runOperation("shift", shiftOne)}">shift</button>
                <button onclick="${() => runOperation("insert middle", insertMiddle)}">insert middle</button>
                <button onclick="${() => runOperation("remove middle", removeMiddle)}">remove middle</button>
            </menu>

            <menu>
                <button onclick="${() => runOperation("push ×50", (target) => pushMany(target, 50))}">push ×50</button>
                <button onclick="${() => runOperation("pop ×50", (target) => popMany(target, 50))}">pop ×50</button>
                <button onclick="${() => runOperation("unshift ×50", (target) => unshiftMany(target, 50))}">unshift ×50</button>
                <button onclick="${() => runOperation("shift ×50", (target) => shiftMany(target, 50))}">shift ×50</button>
                <button onclick="${() => runOperation("replace middle ×10", (target) => replaceMiddleRange(target, 10))}">replace middle ×10</button>
                <button onclick="${() => runOperation("replace middle ×100", (target) => replaceMiddleRange(target, 100))}">replace middle ×100</button>
            </menu>
            ...
            ${measurements.size > 0
                ? html`
                    <div class="results">
                        ...
                        ${Array.from(measurements.values()).map((measurement) => html`
            ...
            <ul>
                ${rows.map((row) => html`<li data-key=${row.identifier}>#${row.identifier} → ${row.value}</li>`)}
            </ul>
        `;
    }),
);
