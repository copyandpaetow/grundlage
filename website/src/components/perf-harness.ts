			...
			${result
				? html`
						<div class="grid">
							...
							<span>fps</span
							><span>${formatNumber(result.framesPerSecond)}</span>
							<span>avg frame</span
							><span>${formatNumber(result.averageFrameMs, 3)} ms</span>
							<span>heap start</span
							><span
								>${result.heapStartMb !== null
									? `${formatNumber(result.heapStartMb)} MB`
									: "n/a"}</span
							>
							<span>heap end</span
							><span
								>${result.heapEndMb !== null
									? `${formatNumber(result.heapEndMb)} MB`
									: "n/a"}</span
							>
