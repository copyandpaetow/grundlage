...
customElements.define(
	"cube-grid",
	render(function* (element: BaseComponent) {
		const xUnits =
			element.getAttribute("units-width") ??
			element.getAttribute("units") ??
			element.getAttribute("width") ??
			"1";
		const yUnits =
			element.getAttribute("units-height") ??
			element.getAttribute("units") ??
			element.getAttribute("height") ??
			"1";

		const zUnits =
			element.getAttribute("units-depth") ??
			element.getAttribute("units") ??
		...
		const xState: DimensionState = {
			size:
				element.getAttribute("size-width") ??
				element.getAttribute("size") ??
				`${100 / parseFloat(xUnits)}cqw`,
			units: xUnits,
			index: 0,
			axis: "x",
		};
		...
		const yState: DimensionState = {
			size:
				element.getAttribute("size-height") ??
				element.getAttribute("size") ??
				`${100 / parseFloat(yUnits)}cqw`,
			units: yUnits,
			index: 0,
			axis: "y",
		};
		...
		const zState: DimensionState = {
			size:
				element.getAttribute("size-depth") ??
				element.getAttribute("size") ??
				`${100 / parseFloat(zUnits)}cqw`,
			units: zUnits,
			index: 0,
			axis: "z",
		};
